...
function create_code($email)
{
    ...
    if ($val == false) {
        ...
        $conn = openCon();
        $sql = "INSERT INTO ehmrs_verification_email (email,code,date_created) VALUES ('$email','$code','$current_date')";
...
{
    $conn = openCon();
    $sql = "DELETE FROM ehmrs_verification_email  WHERE email='$email'";
    if ($conn->query($sql) === TRUE) {
        return;
    } else {
        return "Error: " . $sql . "<br>" . $conn->error;
...
function verify_code($code, $email)
{
    $conn = openCon();
    $current_date = date('Y-m-d H:i:s');
    $sql = "SELECT status,date_created FROM `ehmrs_verification_email` WHERE email='$email' AND code='$code';";
...
function get_current_active($email)
{
    $conn = openCon();
    $current_date = date('Y-m-d H:i:s');
    $sql = "SELECT date_created,code FROM `ehmrs_verification_email` WHERE email='$email' AND status='0';";
    $result = $conn->query($sql);
    if (!$result) {
        echo ("Error description: " . $conn->error);
    }
...
{
    $conn = openCon();
    $sql = "UPDATE ehmrs_users SET email_verify='1' WHERE email='$email'";
    if ($conn->query($sql) === TRUE) {
        return;
    } else {
        return "Error: " . $sql . "<br>" . $conn->error;
